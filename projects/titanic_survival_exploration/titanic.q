...
acc0:{update correct:(n-totg-n)+totn-tots,pc:((n-totg-n)+totn-tots)%totn from ![;();0b;`totn`tots`totg!parse each ("sum n";"(sum;n) fby Survived";"(sum;n) fby ([]",sv[";";string (),x],")")] ?[t;();{x!x}`Survived,x;enlist[`n]!enlist (count;`i)]}
acc0[`Sex`Pclass]

/ brute force groupings
{update cpc:sums pc from `pc xdesc update pc:{(x-y-x)%z}[n;gbyn;sum n] from ![;();0b;enlist[`gbyn]!enlist parse"(sum;n) fby ([]",sv[";";string x],")"] ?[t;();{x!x}`Survived,x;enlist[`n]!enlist (count;`i)]}each (),/:p;
...
`wins xdesc update wins:inc+inc-ng from update inc:n-correct,ng:(sum;n) fby ([]Sex;Pclass;SibSp;Parch;Embarked) from select n:count i,sum correct by Survived,Sex,Pclass,SibSp,Parch,Embarked from update correct:?[(Sex=`female) or not[null Age] and Age<10;1b;0b]=Survived from t
